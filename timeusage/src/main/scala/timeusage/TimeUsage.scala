...
//    val finalDf = timeUsageGroupedSql(summaryDf)
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    ...
    val tailFields = columnNames.tail.map(column => StructField(column, DoubleType, nullable = false))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val listPrimaryNeeds = columnNames.filter( x => x.startsWith("t01") || x.startsWith("t03") || x.startsWith("t11") || x.startsWith("t1801") || x.startsWith("t1803"))
    val listWorking = columnNames.filter( x => x.startsWith("t05") || x.startsWith("t1805"))
    val listOther = columnNames.filter( x => x.startsWith("t02") || x.startsWith("t04") || x.startsWith("t06") || x.startsWith("t07") || x.startsWith("t08") || x.startsWith("t09") || x.startsWith("t10") || x.startsWith("t12") || x.startsWith("t13") || x.startsWith("t14") || x.startsWith("t15") || x.startsWith("t16") || (x.startsWith("t18") && !x.startsWith("t1801") && !x.startsWith("t1803") && !x.startsWith("t1805")))
    (listPrimaryNeeds.map(col(_)), listWorking.map(col(_)), listOther.map(col(_)))
  }
  ...
  def timeUsageSummary(
    ...
    def SumUDF(columns: List[Column], sum: Column): Column = columns match {
      case Nil => sum
      case x::xs => SumUDF(xs, sum + df(x.toString()))
    }
    val primaryNeedsProjection: Column = (SumUDF(primaryNeedsColumns, lit(0.0d)) / 60.0d).as("primaryNeeds")
    val workProjection: Column = (SumUDF(workColumns, lit(0.0d)) / 60.0d).as("work")
    val otherProjection: Column = (SumUDF(otherColumns, lit(0.0d)) / 60.0d).as("other")
    df
